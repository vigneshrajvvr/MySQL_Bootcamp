...
-- CASE exercise
# SELECT title,author_lname, CASE WHEN title LIKE '%stories%' THEN 'Short Stories' WHEN title LIKE '%Just Kids%' OR 'A Heartbreaking Work' THEN 'Memoir' ELSE 'Novel' END AS TYPE FROM books;

-- Bonus excercise
SELECT title,author_lname,CONCAT(COUNT(*),' ','books') AS COUNT FROM books GROUP BY author_lname;